...
from modules.head import DualDetect
    ...
    def __call__(self, model=None) -> str:
        ...
        for m in model.modules():
            if isinstance(m, Classify):
                m.export = True
            if isinstance(m, (Detect, DualDetect, RTDETRDecoder)):  # includes all Detect subclasses like Segment, Pose, OBB
            ...
            if isinstance(m, (Detect, DualDetect)) and imx:
        ...
        if hasattr(model, "names2"):
            self.metadata["names2"] = model.names2
        if dla is not None:
        ...
        if xml:  # OpenVINO
            f[3] = self.export_openvino(dataset=self.args.dataset)
        if coreml:  # CoreML
    ...
    def export_openvino(self, dataset=None, prefix=colorstr("OpenVINO:")):
        ...
        if self.args.int8:
            ...
            quantized_ov_model = nncf.quantize(
                model=ov_model,
                calibration_dataset=nncf.Dataset(self.get_int8_calibration_dataloader(prefix, dataset), self._transform_fn),
