...
class v8DetectionLoss:
    ...
    def __init__(self, model, tal_topk: int = 10, tal_topk2: int | None = None):  # model must be de-paralleled
        ...
        self.bce = nn.BCEWithLogitsLoss(reduction="none", pos_weight=h.class_weights)
        self.hyp = h
    ...
    def get_assigned_targets_and_loss(self, preds: dict[str, torch.Tensor], batch: dict[str, Any]) -> tuple:
        """Calculate the sum of the loss for box, cls, dfl and sequence multiplied by batch size and return foreground mask and
        target indices.
        """
